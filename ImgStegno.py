...
class IMG_Stegno:
    ...
    def main(self, root):
        ...
        title.grid(pady=10)
        title.config(bg='#808080')
        title.grid(row=1)

        encode = Button(frame, text="Encode", command=lambda: self.encode_frame1(
            frame), padx=14, bg='#808080')
        encode.config(font=('Times new roman', 14), bg='#add8e6')
        encode.grid(row=2)
        decode = Button(frame, text="Decode", command=lambda: self.decode_frame1(
            frame), padx=14, bg='#808080')
        decode.config(font=('Times new roman', 14), bg='#add8e6')
        decode.grid(pady=12)
        decode.grid(row=3)
    ...
    def encode_frame1(self, F):
        ...
        label1.config(font=('Times new roman', 25, 'bold'), bg='#808080')
        label1.grid()

        button_bws = Button(F2, text='Select',
                            command=lambda: self.encode_frame2(F2))
        button_bws.config(font=('Times new roman', 18), bg='#add8e6')
        button_bws.grid()
        button_back = Button(
            F2, text='Cancel', command=lambda: IMG_Stegno.back(self, F2))
        button_back.config(font=('Times new roman', 18), bg='#add8e6')
    ...
    def decode_frame1(self, F):
        ...
        label1.config(font=('Times new roman', 25, 'bold'), bg='#808080')
        label1.grid()
        label1.config(bg='#808080')
        button_bws = Button(d_f2, text='Select',
                            command=lambda: self.decode_frame2(d_f2))
        button_bws.config(font=('Times new roman', 18), bg='#add8e6')
        button_bws.grid()
        button_back = Button(d_f2, text='Cancel',
                             command=lambda: IMG_Stegno.back(self, d_f2))
        button_back.config(font=('Times new roman', 18), bg='#add8e6')
        button_back.grid(pady=15)
        button_back.grid()
        d_f2.grid()
    ...
    def encode_frame2(self, e_F2):
        ...
        if not myfile:
            ...
            text_a.grid()
            encode_button = Button(e_pg, text='Cancel',
                                   command=lambda: IMG_Stegno.back(self, e_pg))
            encode_button.config(font=('Times new roman', 14), bg='#add8e6')
            msg = text_a.get("1.0", "end-1c")
            button_back = Button(e_pg, text='Encode', command=lambda: [
                                 self.enc_fun(text_a, my_img), IMG_Stegno.back(self, e_pg)])
            button_back.config(font=('Times new roman', 14), bg='#add8e6')
            button_back.grid(pady=15)
            encode_button.grid()
            e_pg.grid(row=1)
            e_F2.destroy()
    ...
    def decode_frame2(self, d_F2):
        ...
        if not myfiles:
            ...
            label2 = Label(d_F3, text='Hidden msg is :')
            label2.config(font=('Times new roman', 14, 'bold'))
            label2.grid(pady=10)
            ...
            button_back = Button(d_F3, text='Cancel',
                                 command=lambda: self.frame_3(d_F3))
            button_back.config(font=('Times new roman', 14), bg='#add8e6')
            button_back.grid(pady=15)
            button_back.grid()
            d_F3.grid(row=1)
    ...
    def modify_Pix(self, pix, msg):
        dataList = self.generate_Data(msg)
    ...
    def encode_enc(self, newImg, msg):
        w = newImg.size[0]
        (x, y) = (0, 0)

        for pixel in self.modify_Pix(newImg.getdata(), msg):
